...
def build_quadrant(df: pd.DataFrame, full_df: pd.DataFrame | None = None) -> go.Figure:
    ...
    # Two coordinate systems on one log axis, and Plotly does not use the same
    # one for both. SHAPES (the zone rectangles and the median crosshair) take
    # raw data values and Plotly logs them itself; ANNOTATIONS and the axis
    # range take exponents. An earlier comment claimed the opposite for shapes,
    # so every zone and the speed-median line were drawn at x = 10^1.07..10^3.29
    # = 12..1940 read as 1.07..3.29 tok/s — off the left edge of the plot, on
    # every render. Only the captions, being annotations, ever showed.
    zones = [
        (x_min,     med_speed, med_quality, y_max,       "Slow · Smart"),
        (med_speed, x_max,     med_quality, y_max,       "Fast · Smart"),
        (x_min,     med_speed, 0,           med_quality, "Slow · Weak"),
        (med_speed, x_max,     0,           med_quality, "Fast · Weak"),
    ]
    ...
    # Median crosshairs. add_vline is a shape, so it takes the raw median.
    ...
    for provider in providers:
        ...
        fig.add_trace(go.Scatter(
            ...
            # [5] and [6] are the RAW model and provider: the click handler
            # looks the row up by them. [0] and [1] are escaped for the hover
            # text, so a name containing & or < never matched its own row.
    ...
    # Annotations are the exponent-space sink, so the caption x is placed
    # between the logged edges.
    annotations = []
    for x0, x1, y0, y1, label in zones:
        outer_left = x0 == x_min
        ax0, ax1 = math.log10(x0), math.log10(x1)
        annotations.append(dict(
            x=ax0 + (ax1 - ax0) * (0.04 if outer_left else 0.96),
