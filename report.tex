...
live data for 255+ models across 29 providers, refreshed every hour from the
Artificial Analysis API, and makes the full competitive landscape navigable
through eleven specialized views. The system runs entirely in Python using
Plotly Dash, requires no JavaScript build step, and has been deployed at a
public URL since March 2026. This report covers the data pipeline, composite
...
timestamped snapshot in a history directory. Over 30 daily snapshots have
accumulated; these power the Trends tab, which shows how model pricing has
shifted over time.
...
patterns. This collapses two numbers into one for display and sorting while
keeping the full per-direction figures available in the detail table.

\subsection{Local Model and Hardware Data}

The second data source is a manually curated catalog of open-weight models
and GPU hardware specifications. GPU memory bandwidth and VRAM capacity were
...
This catalog is static rather than live — it gets updated when significant
new hardware generations or model families are released. It backs the Run
...
The dashboard has eleven tabs, each targeting a specific question:
...
\textbf{Table} (full sortable list of all 255+ models),
\textbf{Run Local} (VRAM compatibility and speed estimation),
\textbf{Image Gen} (ELO-ranked image generation leaderboard),
\textbf{Video Gen} (ranked video generation comparison), and
\textbf{Trends} (price-over-time from daily historical snapshots).
...
\textbf{Trends: line chart over time.}
The historical snapshot system makes this tab possible. Plotting API prices
over time shows a market that moves fast: several major providers dropped
prices significantly between February and April 2026. A static dataset would
miss this entirely.
...
Three decisions carried through all eleven tabs.
...
Each provider gets both a color and a distinct marker shape, keeping these
consistent across all eleven tabs. A user who learns the color scheme (or the
...
and UI — simplified deployment. There is no Node.js dependency, no separate
API server, and no build process. The application is a single Python process
that Render \cite{render} starts with \texttt{python app.py} and redeploys
automatically on every push to the GitHub repository.

The tradeoff is that some interactions requiring client-side speed in a React
application need a server round-trip in Dash. In practice this was acceptable
because the dataset is small (under 1 MB) and most callbacks return in under
200 ms on the hosted instance.
