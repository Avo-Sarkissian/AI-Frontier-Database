...
% natbib must be loaded BEFORE neurips_2024, which loads it itself in
% author-year mode — incompatible with the numeric \thebibliography below.
...
and \textbf{Video Gen} (video generation ranked by arena Elo, with a mode
control because Artificial Analysis scores text-to-video and image-to-video in
separate pools at separate prices).
...
across all ten tabs. The remaining providers share a neutral ``Other''
