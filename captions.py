...
"Each bubble is one model FAMILY — the best-scoring variant, so five Claude Opus 5 effort tiers "
    ...
    'local': (
        ...
        "be missing and it is not small: Llama 3.1 8B at Q4 needs about 5 GB at 8k "
        "and about 21 GB at its advertised 128k. Attention geometry comes from "
        "each model's published config where this dashboard carries it and is "
        "estimated from parameter count otherwise — the hover says which, and an "
        "estimated figure is good to roughly ±30%. "
        ...
        "compute ceiling, and on every GPU here it is the memory stream, by 3x to "
        "295x. Quantizing does not buy its full byte ratio — FP16 to Q2 is about "
        ...
        "for MoE models. Sessions is the largest number of concurrent streams that "
        "both fit their KV cache in VRAM and hold each stream at or above the "
        "floor you pick (10 tok/s by default, MLPerf's Llama-3.1-8B Server "
        "constraint, ~480 words/minute); the aggregate figure beside it is "
        "decode-only and says nothing about queueing or time-to-first-token. "
        ...
        "length, and see which models fit — with estimated single-stream and "
